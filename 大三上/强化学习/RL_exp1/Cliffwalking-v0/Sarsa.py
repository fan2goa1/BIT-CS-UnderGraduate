...
class Sarsa(object):
    ...
    def update(self, state, action, reward, next_state, next_action, done):  # 根据当前状态、动作、即时奖励、下一状态更新Q表
        Q_now = self.Q_table[str(state)][action] # Q(s, a)的值
        if done: # 终止状态
            Q_target = reward
        else:   # 非终止状态
            Q_target = reward + self.gamma * self.Q_table[str(next_state)][next_action] # Q_target为下一状态下一动作对应的Q值*折扣因子+即时奖励
