...
//    FunctionDefine, FunctionDeclaration, Variable
    public String type;  // 该标识符的数据类型
    public List<String> params = new ArrayList<>();  // 若该标识符为函数，存储函数的参数数据类型列表
}
public class MyVisitor implements ASTVisitor {
    ...
    public void visit(ASTArrayDeclarator arrayDeclarator, String type) throws Exception {
        if( arrayDeclarator.declarator instanceof ASTVariableDeclarator){
            arrayName = ((ASTVariableDeclarator) arrayDeclarator.declarator).identifier.value;
            Symbol symbol = new Symbol();
            symbol.name = "Variable";
            symbol.type = type;
            if( scopeId == 0 ) checkSymbolAdd(arrayName, symbol, symbolTableGlobal);
            else checkSymbolAdd(arrayName, symbol, symbolTableLocal.get(scopeId-1));
        }else{
            this.visit(arrayDeclarator.declarator, type);
        }
        this.visit(arrayDeclarator.expr);
        if(!exprType.equals("int")){
            output("ArrayDeclaration: " + outputName(arrayName) + " array index is not int." ,0);
        }
    }
    ...
    public void visit(ASTArrayAccess arrayAccess) throws Exception {
        if( arrayAccess.arrayName instanceof ASTIdentifier ){
            arrayName = ((ASTIdentifier) arrayAccess.arrayName).value;
        }
        this.visit(arrayAccess.arrayName);
        if( arrayAccess.elements != null ){
            for(ASTExpression element: arrayAccess.elements){
                this.visit(element);
                if(!exprType.equals("int")){
                    output("ArrayAccess: " + outputName(arrayName) + " array index is not int." ,0);
                }
            }
        }
    }

    private boolean checkOpType(String type1, String type2){
        return (type1.equals("int") || type1.equals("double") || type1.equals("float")) && (type2.equals("int") || type2.equals("double") || type2.equals("float"));
    }
    private String maxOp(String type1, String type2){
        if(type1.equals("double") || type2.equals("double")) return "double";
        else if(type1.equals("float") || type2.equals("float")) return "float";
        else return "int";
    }
    private String checkBinaryOp(String type1, String type2, String op) {
        switch (op) {
            case "=":
            case "+=":
            case "-=":
            case "/=":
            case "*=":
                if (!checkOpType(type1, type2)) {
                    output("Operator: " + outputName(op) + " expression's type doesn't match.", 5);
                }
                return type1;
            case "+":
            case "-":
            ...
            case "&&":
                return "int";
            default:
                System.out.println("Other operators haven't committed.");
                return "int";
        }
    }
    ...
    public void visit(ASTContinueStatement continueStatement) throws Exception {
        ...
        output("ContinueStatement: \"continue\" must be in a LoopStatement.", 3);
    }
    ...
    public void visit(ASTFunctionDefine functionDefine) throws Exception {
        ...
        if(!type.equals("void") && !funcReturn){
            output("FunctionDefine: " + outputName(functionName) + " must have a return.", 8);
            funcReturn = true;
        }
        stateList.remove(stateList.size()-1);
